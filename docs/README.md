...
DocForge validates every transition from Shard-Analysis to File-Reduce. File-Reduce is not allowed to run with empty shard-analysis input, preventing weak summaries such as `"Keine Shard-Analysen verfügbar."`.

Outputs:

```text
analysis/pipeline_integrity_report.json
output/run_metadata.json -> pipeline_integrity_report
```

During a run, debug lines show the expected and found shard analyses per file:

```text
[DocForge][Integrity] file=src/app.py expected_shards=2 shard_analyses_found=2
```

Strict behavior is enabled by default. To debug with fallback mode:

```powershell
docforge-enterprise project.zip --allow-missing-shard-analyses
```

See `docs/PIPELINE_INTEGRITY.md`.

## WebGUI Logs

Die WebGUI zeigt Job-Logs sofort nach dem Start an. Details zur Log-Anzeige und zum Polling stehen in [`docs/WEBGUI_LOGS.md`](docs/WEBGUI_LOGS.md).

## Language Selection

DocForge Enterprise can generate German or English documentation.

CLI:

```powershell
docforge-enterprise project.zip --language de
docforge-enterprise project.zip --language en
```

The WebGUI provides a `DE · Deutsch` / `EN · English` selector. The selected language changes the WebGUI labels and is passed to the pipeline so LLM-generated summaries and final documentation are written in the selected language.

Code identifiers, file paths and symbols remain unchanged.

See `docs/I18N_LANGUAGE.md`.
